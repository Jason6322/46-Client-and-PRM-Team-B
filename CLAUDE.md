...
| `frontend/src/components/shared/`            | `Card { title?, className?, children }`, `ErrorBoundary`, `LoadingSpinner`, `FullPageSpinner`, `EmptyState { title, description?, icon?, action? }`                                                           | Surfaces, loading/empty/error states                            |
...
| File                               | Exports                                                                                                                                                                                                                                                 | Use for                                                       |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `actions/organisations.actions.ts` | `createOrganisation`, `getOrganisation`, `listOrganisations`, `listArchivedOrganisations`, `updateOrganisation`, `changePipelineStage`, `setNextAction`, `archiveOrganisation`, `restoreOrganisation` — all `ActionResult`                              | Every read and write (Admin SDK, `requireAuth()` first)       |
| `types.ts`                         | `OrganisationListItem` — `Organisation` with every Timestamp as millis                                                                                                                                                                                  | What components receive; Timestamps can't cross to the client |
| `constants.ts`                     | `ORGANISATION_TYPES`, `PIPELINE_STAGES` (12, from the dashboard chart), `DEFAULT_PIPELINE_STAGE`, `RELATIONSHIP_STATUSES` (`Active`, `Prospect`)                                                                                                        | Enums                                                         |
| `followUp.ts`                      | `dateOnlyToDate`, `millisToDateOnly`, `isOverdue`, `isDueToday`, `isDateOnly`                                                                                                                                                                           | Due dates — stored at 12:00 UTC on the day                    |
| `retention.ts`                     | `RETENTION_DAYS` (30), `purgeDateFor`, `isPastRetention`                                                                                                                                                                                                | Archive retention — **nothing purges yet**                    |
| `lib/validations/organisation.ts`  | Storage: `createOrganisationSchema`, `updateOrganisationSchema` (partial — omits absent keys, applies no defaults), `pipelineStageSchema`, `nextActionSchema` · Form: `organisationFormSchema`, `toCreateOrganisationInput`, `toOrganisationFormValues` | Form schema speaks strings; storage schema speaks nulls       |
| `components/`                      | `OrganisationsTable`, `OrganisationForm { organisation? }` (create and edit), `OrganisationDetail`, `OrganisationDetailsCard`, `ContactsCard`, `PipelineStageSelect`, `NextActionEditor`, `ArchiveOrganisationButton`, `ArchivedOrganisationsTable`     | UI                                                            |

Archiving is a soft delete (`deletedAt`) and is separate from the `Archived` pipeline stage. Lists fetch the collection once and filter in memory — no composite indexes are needed. `organisations` security rules are not written yet, so client-SDK reads are denied by the default-deny block; the Server Actions are unaffected.
...
Pages: `/` · `/auth/signin` · `/auth/signup` · `/dashboard` · `/profile` · `/settings` · `/organisations` · `/organisations/new` · `/organisations/[id]` · `/organisations/[id]/edit` · `/organisations/archived` (route groups `(auth)`, `(dashboard)`). `/relationships`, `/pipeline`, `/meetings` and `/opportunities` exist for the nav but render nothing — no wireframes yet.
